...
function clear() {
  ...
  function move() {
    ...
    var a = father - 4;
    if (a >= 0 && a <= 15) {
      if (cell[a].childNodes[0].className == "grey") { //找到交换对象
    ...
    var b = father + 4;
    if (b >= 0 && b <= 15) {
      if (cell[b].childNodes[0].className == "grey") {
    ...
    var c = father - 1;
    if (c >= 0 && c <= 15) {
      if ((cell[c].childNodes[0].className == "grey") && (father % 4 != 0)) {
        cell[c].childNodes[0].className = this.className;
        alert(this.className + cell[c].childNodes[0].className);
        this.className="grey";
        return;
      }
    }
    ...
    var d = father + 1;
    if (d >= 0 && d <= 15) {
      if ((cell[d].childNodes[0].className == "grey") && (father % 4 != 3)) {
        cell[d].childNodes[0].className = this.className;
        cell[d].childNodes[0].id=this.id;
        alert(this.className + cell[d].childNodes[0].className);
        this.className="grey";
        this.id=15;
        return;
      }
  ...
  function isOver() {
    for (var i = 0; i < 16; i++) {
      if ($(".cell")[i].childNodes[0].className != "grey") {
